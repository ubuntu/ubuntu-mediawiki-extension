/**
 * Code block enhancements for the Ubuntu wiki.
 *
 * Enhances .ubuntu-code-block markup (produced by on-wiki templates) with a
 * "Copy" button that copies the block's code to the clipboard. Buttons are
 * bound on page load, on wikipage.content updates, and via a MutationObserver
 * so late-inserted blocks (e.g. from gadgets or the API) are also enhanced.
 *
 * Accessibility:
 * - The button gets an aria-label (overridable per block/button via
 *   data-copy-label / data-copied-label attributes).
 * - A visually hidden role="status" live region announces successful copies.
 * - When the next focusable element after a block uses a positive tabindex,
 *   the button inherits it so keyboard focus order stays sensible.
 *
 * Requires the Clipboard API; without it the module is a no-op and code
 * blocks render unenhanced.
 */
...
function getCopyLabel( button ) {
	...
	if (
		...
		return mw.msg( 'ubuntu-codeblock-copy-captcha-command' );
	}

	return mw.msg( 'ubuntu-codeblock-copy' );
}
...
function bindBlocks( root ) {
	...
	blocks.forEach( ( block ) => {
		...
		if ( !block.querySelector( BUTTON_SELECTOR ) ) {
			...
			button.type = 'button';
			button.textContent = mw.msg( 'ubuntu-codeblock-copy' );
...
function init() {
	...
	document.addEventListener( 'click', ( event ) => {
		...
		const copyButtonLabel = mw.msg( 'ubuntu-codeblock-copy' ),
			copyLabel = getCopyLabel( button ),
			copiedLabel =
				button.dataset.copiedLabel || mw.msg( 'ubuntu-codeblock-copied' );
