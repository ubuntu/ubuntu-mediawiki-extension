...
- **Entry files are import-only.** `common.less` / `minerva.less` /
  `vector.less` only `@import`; actual declarations live in
   `components/ubuntu/*.less` (extension-owned styles),
   `components/mediawiki/*.less` (shared MediaWiki markup adaptations), and
   `vendor/*.less` (generated design tokens).
...
- **Fonts live in `resources/fonts/`** and are declared in
   `ext.ubuntu.styles/components/ubuntu/Fonts.less` with per-script `unicode-range`
  subsets, so browsers only download what a page uses. The base
  (`Ubuntu Sans`) and mono (`Ubuntu Sans Mono`) stacks are applied in
   `ext.ubuntu.styles/components/mediawiki/Typography.less`, so both skins get them
  from this module.
